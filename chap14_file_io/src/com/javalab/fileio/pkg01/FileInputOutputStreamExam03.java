...
// fileInputStream 과 fileOutputStream을 통한 이미지 복사(copy)
public class FileInputOutputStreamExam03 {
	public static void main(String[] args) {
		...
		try {
			...
			while (true) {
				...
				if (num == -1)
					break;
				fos.write(data, 0, num); // data 속 0개부터 읽어들인 갯수(num)만큼 출력(씀)
			}
			...
			if (fis != null) {
				try {
					...
					System.out.println("FileInput Stream close Exception :" + e2.getMessage());
				}
			}
			if (fos != null) {
				try {
					...
					System.out.println("FileOutput Stream close Exception :" + e2.getMessage());
				}
			}
		}
