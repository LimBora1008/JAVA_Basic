...
public class ImplementAction extends JFrame implements ActionListener {
	...
	public ImplementAction() {
		...
		/*
		 * 버튼 액션 리스너(이벤트 핸들러) 부착 - 익명 구현 객체로 만들지 않고 별도의 메소드로 분리 - 별도의 분리된
		 * actionPerformed() 메소드 호출 - this : 파라미터로 자기 자신을 전달함
		 */
		buttonObj.addActionListener(this);

		// JFrame을 상속받았으므로 add()메소드 바로 사용
		add(buttonObj);
		add(tf);
		add(labelObj);
