...
public class TeaList extends JFrame implements ActionListener{
     ...
     * @param db
     */
    public TeaList(DataBase db) {
    	// 데이터베이스 전담 클래스의 객체를 전달받아서 내가 선언한 변수에 저장함
    	this.db = db;
    	
        // 테이블 세팅
        String[] columnNames = {"상품명", "가격"};
        List<Tea> tealList = db.TeaList;
        Object[][] data = new Object[tealList.size()][6];
        
        for (int i = 0; i < tealList.size(); i++) {
        	Tea tea = tealList.get(i);
